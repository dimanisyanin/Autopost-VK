    ...
    while True:
        try:
            for group in vk_group:
                print("Group " + group)
                PARAMS = {
                    "owner_id":group,#-45441631 -74761029
                    "count":count_wallGet,
                ...
                wallGet = requests.get(url="https://api.vk.com/method/wall.get", params=PARAMS)
                jsonWallGet = wallGet.json()
                #write_json(jsonWallGet, 'jsonWallGet.json')
                #time.sleep(555)
...
def photos_save(file_name):
    try:
        ...
        r = requests.get('https://api.vk.com/method/photos.save', params = {'album_id': album_id,
                                                                            'group_id': group_id,
                                                                            'server': ur['server'],
...
def get_upload_server():
    try:
        r = requests.get('https://api.vk.com/method/photos.getUploadServer', params = {'album_id': album_id,
                                                                                       'group_id': group_id,
                                                                                       'access_token': VK_API_STADALONE,
                                                                                       'v': VK_API_V}).json()
...
def download_img(url):
    try:
        p = requests.get(url)
        file_name = 'temp_img/' + str(random.randint(1, 999999999999)) + '.jpg'
        ...
        file_name2 = str(random.randint(1, 999999999999)) + '.jpg'
